...
def GenerarArbolResultados():
    ...
    # Comprueba si el automata tiene transiciones epsilon
    if "E" in Alfabeto:
        epsilon = True
    else:
        epsilon = False
    ...
    # Ciclo que validará la cadena insertada por el usuario en el AFN
    for i in cadena:
        ...
        # Si el simbolo a evaluar forma parte del alfabeto, este se usara para generar el arbol
            ...
            # Proceso que se realiza solo si el AFN tiene transiciones epsilon
            if epsilon:
                # Se eveluaran primero todas las transiciones epsilon existentes previas a una transicion con algún otro simbolo
                # print(f" ------ PilaNodos: {PilaNodos}")
                columna = Alfabeto.index("E")
                pilaAux = PilaNodos[:] # Pila Auxiliar donde se enliustaran todos los posibles nodos que puedan tener una transicion epsilon
                PilaNodos.clear()
                bandera = True # Bandera que si es True significa que aun hay transiciones epsilon que pueden ser evaluadas antes de una transicion con un simbolo. De lo contrario sera False
                # print(f" ---- pilaAux: {pilaAux}")

                # Este ciclo servira para obtener todos los estados resultantes de una transicion epsilon.
                # El ciclo no se dentendrá hasta que ya no haya posibilidad de realizar transiciones epsilon
                while bandera:
                    # En caso de la pila de estados con transicion epsilon este vacia, saldremos del ciclo sin hacer nada
                    if len(pilaAux) == 0:
                        bandera = False
                        continue

                    # Se obtiene el estado, cuyas transiciones epsilon serán evaluadas
                    edo_actual = pilaAux.pop()
                    # Se obtiene la la ubicacion de la fila de la tabla de transiciones donde se encuentra la transicion epsilon de un estado,
                    ...
                    # del error (esto se hace tomando en cuenta que algun estado de error puede venir dentro de la píla de nodos,
                    # resultante de la evaluacion de alguna transicion no epsilon anterior)
                    if not(edo_actual.name == "_err"):
                        for k in Transiciones[fila][columna]:
                            # Si el resultado de la transicion epsilon lleva a un estado de error, significa que dicho estado tiene transicion con otro simbolo.
                            # Como esto significa que en este estado puede haber una transicion con otro simbolo, el estado de error que produce no se añade al arbol.
                            # Para que la evaluación de este nodo con otro simbolo pueda ser realizada, este se enlista nuevamente en la Pila de Nodos, para
                            # así poder colcer a ser evaluado en el ciclo siguiente
                            if k == "_err":
                                PilaNodos.append(edo_actual)
                            # En caso de que la transicion epsilon sea válida, se obtienen todos los estados resultantes de dicha transicion
                            else:
                                Nodos[id] = (Node(k, parent=edo_actual))
                                pilaAux.append(Nodos[id])
                                id += 1

                    # Cuando la lista de nodos que pueden tener transicion epsilon este vacía. Significa que se han hecho todas las transiciones esilon posibles
                    # previas a la evaluacion de transiciones con simbolos no epsilon. Por lo cual al llegar a este punto, se cambiara la bandera a False, y de
                    # esta manera salir del ciclo de transiciones epsilon y continuar con la evaluacion de las demas transiciones
                    if len(pilaAux) == 0:
                        bandera = False
                    # print(f" -- PilaNodos: {PilaNodos}")
                    # print(f" -- pilaAux: {pilaAux}")
            ...
            # Se obtiene la ubicacion de la columna de la tabla de transiciones donde se encuentra la transicion del estado no epsilon,
            # usando como referencia el simbolo obtenido
            columna = Alfabeto.index(i)
            cont = len(PilaNodos)

            # Este ciclo servira para obtener todos los estados resultantes de una transicion no epsilon
            for j in range(cont):
                # Se obtiene el estado, cuyas transiciones serán evaluadas
                ...
                # Para evitar que se repita el estado de error una vez una transicion ha llegado a este por primera vez, los
                # estados resultantes de una transicion solo se contaran dentro de el arbol si es que no son stados derivados
                # del error (este proceso solo se incluye para el caso de que se inserte un automata sin transicion epsilon, pues
                # dicho proceso hace esta evaluacion de igual manera)
                if not (edo_actual.name == "_err"):
                    for k in Transiciones[fila][columna]:
                        Nodos[id] = (Node(k, parent = edo_actual))
                        pilaAux.append(Nodos[id])
    ...
    # Proceso que se realiza solo si el AFN tiene transiciones epsilon
    if epsilon:
        # Una vez el ciclo de evaluacion de la cadena haya finalizado, se hara una ultima evaluación de transiciones epsilon en el AFN
        # Esto se realiza para asi poder mostrar el arbol de transiciones comnpleto, para los casos donde aun existan transiciones epsilon que
        # se pueden recorrer tras la evaluación del ultimo caracter de la cadena en el AFN. Este proceso toma mayor importancia en caso de que
        # la cadena sea valida en el AFN, pero para llegar al estado final sera necesario recorrer una o mas transiciones AFN después de la
        # evaluación del ultimo caracter de la cadena.
        # --NOTA: El algoritmo presentado dentro de la condicional es exactamente el mismo que el presentado en el ciclo de evaluacion de la cadena.
        if len(PilaNodos) != 0:
            # Este ciclo servira para obtener todos los estados resultantes de una transicion epsilon.
            # print(f" ------ PilaNodos: {PilaNodos}")
            columna = Alfabeto.index("E")
