/*
Practica 2: Scanner ARP (vesión con base de datos en MySQL)
Redes de Computadoras
Hecho por Mauro Sampayo Hernández

Compilación: gcc -o Practica2sql $(mysql_config --cflags) Practica2sql.c $(mysql_config --libs) -lm
*/
...
void AlmacenarSQL()
{
	...
	unsigned char striporigen[15]; 
	unsigned char stripdestino[15];
	unsigned char strmacdestino[18];
	int qstate=0;
	
	sprintf(striporigen, "%d.%d.%d.%d", DireccionRed[0], DireccionRed[1], DireccionRed[2], DireccionRed[3]);
	sprintf(stripdestino, "%d.%d.%d.%d", IPdestino[0], IPdestino[1], IPdestino[2], IPdestino[3]);
	sprintf(strmacdestino, "%.2x:%.2x:%.2x:%.2x:%.2x:%.2x", trama_ARP_respuesta[6], trama_ARP_respuesta[7], trama_ARP_respuesta[8], trama_ARP_respuesta[9], trama_ARP_respuesta[10], trama_ARP_respuesta[11]);

	//Concatenamos la IP de la trama recibida y su MAC a una cadena con un insert into para meterlo a la base
	char* insert;
	insert=(char*)malloc(200);
	sprintf(insert,"INSERT INTO DatosESCOM (IP_que_envia, IP_que_recibe, MAC_IP_que_recibe) VALUES ('%s', '%s', '%s');",striporigen, stripdestino, strmacdestino);
	qstate=mysql_query(base_datos, insert);//se realiza la insercion de los elementos a la base de datos
...
int main()
{
	...
	else
	{
		...
		for(i=1; i<255; i++)
		{
			...
			RecibeARPRespuesta(packet_socket, trama_ARP_respuesta);//editar
		}
	}
