# Práctica 07. Cosenos Directores. (SSIS-3D)
...
class Prac07(QMainWindow):
    ...
    def cerrarVentana(self):  
        if hasattr(self, 'graficaCos'):
            self.graficaCos.cerrarGraf()
        self.parent().show()
        self.close()
    ...
    def graficarCosenos(self):
        ...
        if (self.sBox_magnitud.text() == '0.00'):
            ...
            # Ejes del Vector
            # α es el angulo entre el vector y i
            rx = magnitud * np.cos(alpha)
            # β es el angulo entre el vector y j
            ry = magnitud * np.cos(beta)
            # ɣ es el angulo entre el vector y k
            rz = magnitud * np.cos(gamma)
            ...
            self.graficaCos.graficar_vector_cos(rx, ry, rz)   # Trabajamos con la gráfica
