...
import numpy as np
from Graficas_Practica03 import Canvas_Grafica

class Prac03(QMainWindow):
    def __init__(self, parent=None):
        super(Prac03, self).__init__(parent)
        loadUi('Interfaces\Practica03.ui', self)
    ...
    def cerrarVentana(self):
        if hasattr(self, 'graficaCarteEsfe'):
            self.graficaCarteEsfe.cerrarGraf()
        if hasattr(self, 'graficaEsfeCarte'):
            self.graficaEsfeCarte.cerrarGraf()
        self.parent().show()
        self.close()
    ...
    def graficar_carte_esfe(self):     # Función para graficar Cartesianas -> Esféricas
        ...
        # Valores en Radianes
        self.r_grad.setText(str(r))
        self.theta_rad.setText(str(theta))
        self.phi_rad.setText(str(phi))
        # Valores en Grados
