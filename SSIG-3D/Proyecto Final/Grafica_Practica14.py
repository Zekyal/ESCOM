...
class Tiro_Rifle(FigureCanvas):
    ...
    #------------ Grafica Tiro con Rifle ----------------------------------------
    def graficarTiroRifle(self, zinit, xf, yf, zobj, v_vox, v_voy, v_voz, v_gamma, v_betha, v_alpha, vo):    # Se piden las variables de otra clase para poder trabajarlos.
        ...
        v_gamma.display(round(np.rad2deg(np.arccos(voz/vo)), 2))
        v_betha.display(round(np.rad2deg(np.arccos(voy/vo)), 2))
        v_alpha.display(round(np.rad2deg(np.arccos(vox/vo)), 2))
