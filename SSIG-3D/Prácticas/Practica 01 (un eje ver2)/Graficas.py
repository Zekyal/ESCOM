...
class Canvas_grafica(FigureCanvas):
    ...
    def graficarPosicionNormal(self, pos01, tiem01):    # Se piden las variables de otra clase para poder trabajarlos.
        ...
        #self.ax.spines["right"].set_position("zero")    # Mueve la espina derecha de la gráfica al centro de la gráfica
        #self.ax.spines["top"].set_position("zero")      # Mueve la espina superior de la gráfica al centro de la gráfica
        ...
        animationPos = FuncAnimation(self.fig, func=animation_frame_Pos01, frames=np.arange(0, 99, 1), interval=100, repeat = True) # Función para aplicar la animación
...
class Canvas_grafica2(FigureCanvas):
    ...
    def graficarVelocidadNormal(self, pos01, vel01, tiem01):
        ...
        animationVel01 = FuncAnimation(self.fig, func=animation_frame_Vel01, frames=np.arange(0, 99, 1), interval=100, repeat = True)    # Función para aplicar la animación
        animationVel01.save('Practica01_grafs\Velocidad01.gif')   # Guarda la animación en un archivo de tipo ".gif"
        self.fig.tight_layout()
        self.fig.canvas.draw()
        #self.fig.canvas.flush_events()  # Le da tiempo para pensar al programa
