...
class MyApp(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def graficacion(self):
        ...
        #self.graf_pos_nor.addWidget(self.graficaPosNor)     # Agregamos el widget a cada una de los Layout, ya no agregar en cambio
        #self.graf_vel_nor.addWidget(self.graficaVelNor)     # de utilizar los gif
        #self.graf_ace_nor.addWidget(self.graficaAceNor)
        #self.graf_pos_vect.addWidget(self.graficaPosVec)
        #self.graf_vel_vect.addWidget(self.graficaVelVec)
        #self.graf_ace_vect.addWidget(self.graficaAceVec)
        ...
        self.graf_pos01_gif.setMovie(self.movie)
        #self.movie.start() 

        self.pushButton_start_anim.setEnabled(True)  # Habilita el botón de comenzar animación.
        self.pushButton_stop_anim.setEnabled(True)  # Habilita el botón de detener animación.
        self.pushButton_next_anim.setEnabled(True)  # Habilita el botón de siguiente animación.

        #self.timer = QTimer(self)
        #self.timer.start(5000)  # 1000 = 1 segundo. Ajustar según corresponda. 13000
    ...
    def stop_animation(self):
        ...
        self.movie.stop()
        #self.movie.setFileName("Practica01_grafs\Velocidad01.gif")
        #self.graf_vel01_gif.setMovie(self.movie)
        #self.movie.start()
        #self.timer.start(5000)  # 1000 = 1 segundo. Ajustar según corresponda. 13000
        #self.timer.timeout.connect(self.new_animation2)
        
    def animacion_vel01(self):
        self.movie.setFileName("Practica01_grafs\Velocidad01.gif")
        self.graf_vel01_gif.setMovie(self.movie)
        print(self.movie.nextFrameDelay())
...
class Canvas_grafica(FigureCanvas):
    ...
    def graficarPosicionVector(self, tiem01, pos01):
        ...
        for i in range(0,19):   # Se realiza tantos valores hay en la tabla_pos
            self.ax.plot(tiem01, pos01)
            self.ax.quiver(int(tiem01[i]), int(pos01[i]), int(tiem01[i+1]), int(pos01[i+1]), color= ['blue'])   # Hace los vectores en la gráfica
            #print("De [" + str(tiem01[i]) +"," +str(pos01[i]) + "].  Vamos a [" + str(tiem01[i+1]) + "," + str(pos01[i+1])+ "]") # prueba de movimiento
